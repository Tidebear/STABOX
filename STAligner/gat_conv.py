...
class MNNConv(MessagePassing):
    def __init__(self, in_channels: Union[int, Tuple[int, int]],
                 out_channels: int, num_relations: int,  aggr: str = 'mean', **kwargs):  # yapf: disable

        super(MNNConv, self).__init__(aggr=aggr, node_dim=0, **kwargs)
    ...
    def forward(self, x: Union[OptTensor, Tuple[OptTensor, Tensor]],
                edge_index: Adj, edge_type: OptTensor = None, edge_type_weight: OptTensor=None):
        r"""
        Args:
        ...
        for i in range(self.num_relations):
            ...
            else: #注意，这里的传播只更新目标节点，所以正向和反向边都要加上
                # 这个传播函数，只更新对应边的节点嵌入，其他节点嵌入就直接为0！！
                h = self.propagate(tmp, x=x_l, size=size) #, edge_weight=edge_weight
                if edge_type_weight is not None:
                    out = out + edge_type_weight[i] * (h @ weight[i])
                else:
                    out = out + (h @ weight[i])
