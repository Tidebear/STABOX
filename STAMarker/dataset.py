...
from STAMarker.utils import compute_spatial_net, stats_spatial_net, compute_edge_list
...
class SpatialDataModule(pl.LightningDataModule):
    ...
    #                  show_net_stats: bool = False, min_cells=0, min_counts=None) -> None:
    ...
    def prepare_data(self, n_top_genes: int = 3000, rad_cutoff: float = 50,
        ...
        if self.flag:
            ...
            sc.pp.highly_variable_genes(self.ann_data, flavor="seurat_v3", n_top_genes=n_top_genes)
            self.ann_data = self.ann_data[:, self.ann_data.var['highly_variable']]
            self.ann_data.var_names_make_unique()
            sc.pp.normalize_total(self.ann_data, target_sum=1e4)
            sc.pp.log1p(self.ann_data)
            compute_spatial_net(self.ann_data, rad_cutoff=rad_cutoff)
    ...
    def prepare_data_new(self, n_top_genes: int = 3000, rad_cutoff: float = 50,
                    show_net_stats: bool = False) -> None:
        self.prepare_data_per_node = True
        self._log_hyperparams = True
        sc.pp.calculate_qc_metrics(self.ann_data, inplace=True)
        print(self.ann_data.shape)
    ...
    def prepare_data_subset(self, genes_list, rad_cutoff=150):
        """
        Prepare data for a subset of genes
        genes_list: list of gene names, must be a subset of `self.ann_data.var.index`
        rad_cutoff: radius cutoff for spatial network
        """
        self.ann_data = self.ann_data[:, genes_list]
        sc.pp.normalize_total(self.ann_data, target_sum=1e4)
        sc.pp.log1p(self.ann_data)
        compute_spatial_net(self.ann_data, rad_cutoff=rad_cutoff)
        # ---------------------------- generate data ---------------------
