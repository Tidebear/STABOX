...
from STAMarker.models import intSTAGATE, StackClassifier
from STAMarker.utils import plot_consensus_map, compute_consensus_matrix, Timer, plot_clustered_consensus_matrix
from STAMarker.dataset import SpatialDataModule
from STAMarker.modules import STAGATEClsModule
...
def make_spatial_data(ann_data):
    ...
    ann_data.X = ann_data.X.A if issparse(ann_data.X) else ann_data.X
    print(type(ann_data.X))
...
class STAMarker:
    ...
    def set_version_dirs(self, version_dirs):
        self.version_dirs = version_dirs
        self.n = len(version_dirs)
        logging.info("Set version_dirs. STAMarker object now contains {} version directories".format(self.n))
    ...
    def consensus_clustering(self, n_clusters, name="cluster_labels.npy", plot=True):
        ...
        print("compute labels: {:.2f}".format(time.time() - st))
    ...
    def consensus_clustering2(self, n_clusters, name="cluster_labels.npy", plot=True):
        label_files = [os.path.join(dir, name) for dir in self.version_dirs]
        labels_list = list(map(lambda file: np.load(file), label_files))
        labels_list = np.vstack(labels_list)
        self.logger.info("Perform consensus clustering on {} clustering  results".format(len(self.version_dirs)))
        import time
        st = time.time()
        cons_mat = compute_consensus_matrix(labels_list)
        print("compute consensus matrix: {:.2f}".format(time.time() - st))
    ...
    def compute_smaps(self, data_module, return_recon=True):
        smaps = []
        if return_recon:
            recons = []
        for version_dir in self.version_dirs:
    ...
    def compute_smaps_mean(self, data_module, return_recon=True):
        """
        compute smaps mean by moving average and avoid use list to store smaps
        """
        smaps_mean = 0
        if return_recon:
            recons_mean = 0
        for i, version_dir in enumerate(self.version_dirs):
            if return_recon:
                smap, recon = self._compute_smap(data_module, version_dir, return_recon=return_recon)
                smaps_mean = (smaps_mean * i + smap) / (i + 1)
                recons_mean = (recons_mean * i + recon) / (i + 1)
            else:
                smap = self._compute_smap(data_module, version_dir, return_recon=return_recon)
                smaps_mean = (smaps_mean * i + smap) / (i + 1)
        self.logger.info("Finished computing {} smaps".format(len(self.version_dirs)))
        if return_recon:
            return smaps_mean, recons_mean
        else:
            return smaps_mean
        
    def get_sailiency_scores(self, data_module, smaps_mean=None, labels=None):
        if smaps_mean is None:
            smaps_mean = self.compute_smaps_mean(data_module, return_recon=False)
        if labels is None:
            try:
                labels = self.consensus_labels
            except:
                raise ValueError("No consensus labels found. Run consensus_clustering first")
        scores = pd.DataFrame(index=data_module.ann_data.var.index)
        unique_labels = np.unique(labels)
        n_clusters = len(unique_labels)
        for l in unique_labels:
            scores[l] = np.linalg.norm(smaps_mean[labels == l, :], axis=0)
        return scores
    

    def select_svgs(self, scores, alpha=1.5):
        """
        apply filter function to each column of scores > column.mean() + alpha * column.std()
        """
        svgs = scores.apply(lambda x: x > x.mean() + alpha * x.std(), axis=0)
        # compute the rows having more than 1 svg
        svgs = svgs.apply(lambda x: x.sum() > 1, axis=1)
        return svgs
