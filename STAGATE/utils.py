...
def Visium_data_process(file_path, count_file, ground_truth_path):
    adata = sc.read_visium(path=file_path, count_file=count_file)
    adata.var_names_make_unique()
    sc.pp.highly_variable_genes(adata, flavor="seurat_v3", n_top_genes=3000)
    # the version of scanpy does not fit python==3.8, need python==3.7
    sc.pp.normalize_total(adata, target_sum=1e4)
    sc.pp.log1p(adata)
    Ann_df = pd.read_csv(ground_truth_path, sep='\t', header=None, index_col=0, encoding="gbk")
    Ann_df.columns = ['Ground Truth']
    adata.obs['Ground Truth'] = Ann_df.loc[adata.obs_names, 'Ground Truth']
    return adata


def Slideseq_data_process(file_path, exp_file, loc_file):
    counts = pd.read_csv(os.path.join(file_path, exp_file), sep='\t', index_col=0, encoding="gbk")
    coor_df = pd.read_csv(os.path.join(file_path, loc_file), index_col=0, encoding="gbk")
...
def Stereoseq_data_process(file_path, exp_file, loc_file, used_file):
    counts = pd.read_csv(os.path.join(file_path, exp_file), sep='\t', index_col=0, encoding="gbk")
    coor_df = pd.read_csv(os.path.join(file_path, loc_file), sep='\t', encoding="gbk")
    print(counts.shape, coor_df.shape)
    ...
    plt.title("")
    plt.axis('off')
    used_barcode = pd.read_csv(os.path.join(file_path, used_file), sep='\t', header=None)
